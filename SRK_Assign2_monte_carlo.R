...
simu_norm <- function(n){
  ...
  y <- rnorm(n,2,3)  # simulated data from Normal(2,3).
  low <- min(y)
  upp <- max(y)
  
  # Get Kernel density estimate, with bandwidth as nrd0 and kernel as gaussian, 
  # since the data are symmetric.
  fKde <- kdensity(y,bw='nrd0',kernel='gaussian',support=c(low,upp))
  ...
  fMple <- ssden(~y,domain=data.frame(y=c(low,upp)))
  
  # Plot the true density and estimated densities, the domain is set based on the
  # data we simulated as well.
  y1 <- seq(low,upp,len=500)
  plot(y1, dnorm(y1, 2, 3),type = "l", lwd = 2, ylim = c(0,0.16), xlim=c(low,upp),
       ylab = 'p.d.f',xlab = 'y',bty = 'n')  # Plot the true density.
  lines(y1, fKde(y1), lwd = 1, col = 'green')  # Plot kernel density estimate.
  lines(y1, dssden(fMple, y1), lwd = 1, col = 'red')  # Plot maximum penalized likelihood estimate
...
simu_exp <- function(n){
  ...
  y <- rexp(n,2)  # Simulated data from exponential(2)
  low <- min(y)
  upp <- max(y)
  ...
  fKde <- kdensity(y,bw='ucv',kernel='gamma',support=c(low,upp))
  # Get maximum penalized likelihood estimate. Theoretically, the true domain for 
  # the density is unknown, so set the domain based on the data simulated.
  fMple <- ssden(~y,domain=data.frame(y=c(low,upp)))
  ...
  # data we simulated as well.
  y1 <- seq(low,upp,len=500)
  plot(y1, dexp(y1, 2),type="l", lwd=2, ylim=c(0,2.5), xlim=c(low,upp), ylab = 'p.d.f',
       xlab = 'y', bty='n')  # Plot the true density.
  lines(y1, fKde(y1), lwd = 1, col = 'green')  # Plot kernel density estimate.
  lines(y1, dssden(fMple,y1), lwd = 1, col = 'red')  # Plot maximum penalized likelihood estimate.
...
simu_beta <- function(n){
  ...
  y <- rbeta(n,3,1)  # Simulated data from beta(3,1).
  low <- min(y)
  upp <- max(y)
  ...
  fKde <- kdensity(y,bw='ucv',kernel='beta',support=c(low,upp))
  # Get maximum penalized likelihood estimate. Theoretically, the true domain for
  # the density is unknown, so set the domain based on the data simulated.
  fMple <- ssden(~y,domain=data.frame(y=c(low,upp)))
  ...
  # data we simulated as well.
  y1 <- seq(low,upp,len=500)
  plot(y1, dbeta(y1,3,1),type="l", lwd=2, ylim=c(0,4), xlim=c(low,upp), ylab = 'p.d.f',
       xlab = 'y', bty='n')  # Plot the true density.
  lines(y1, fKde(y1), lwd = 1, col = 'green')  # Plot kernel density estimate.
  lines(y1, dssden(fMple, y1), lwd = 1, col = 'red')   # Plot maximum penalized likelihood estimate.
